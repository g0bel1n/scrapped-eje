...
def main():
    ...
    start = time.time()
    n = runImmoDataScrapping(filename= 'immo_data.csv')
    duration = time.time()-start
    logger.info(f'Scrapped {n} items in {duration/60.} minutes. ({str(duration/n).split(".")[0]} seconds per it)')
    
def runImmoDataScrapping(filename: str):
    driver = webdriver.Chrome()
    pageNumber=1
    url = 'https://www.immo-data.fr/explorateur/transaction/recherche?minprice=0&maxprice=5000000&minpricesquaremeter=0&maxpricesquaremeter=40000&propertytypes=0%2C1%2C2&minmonthyear=Janvier%202014&maxmonthyear=D%C3%A9cembre%202021&nbrooms=1%2C2%2C3%2C4%2C5&minsurface=0&maxsurface=400&minsurfaceland=0&maxsurfaceland=50000&center=-0.57918%3B44.837789&zoom=11'
