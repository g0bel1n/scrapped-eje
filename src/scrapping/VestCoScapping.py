...
def main():
    ...
    parser.add_argument(
        "--fn", type=str, default="vestco_", help="name of the output CSV file"
    )
...
def runVintedScrapping(n: int, query: str, filename: str):
    ...
    while len(data["Designer"]) < n:
        elements = driver.find_elements(
            by=By.CSS_SELECTOR, value="vc-ref.productSnippet__imageContainer [href]"
        )
        ...
        for link in links:
            ...
            if driver.current_url.endswith("shtml"):
                description = driver.find_element(
                    by=By.CLASS_NAME,
                    value="product-seller-description_sellerDescription__SnSkU",
                ).get_attribute("textContent")

                titles = driver.find_elements(
                    by=By.CSS_SELECTOR,
                    value="span.product-description-list_descriptionList__property__21dco",
                )
                values = driver.find_elements(
                    by=By.CSS_SELECTOR,
                    value="span.product-description-list_descriptionList__value__J3Z9l",
                )
                dictVal = {
                    title.get_attribute("textContent"): value.get_attribute(
                        "textContent"
                    )
                ...
                for col in colsInDetailsList:
                    data[col].append(
                        dictVal[col + " :"] if col + " :" in dictVal else np.nan
                    )

                try:
                    prix = driver.find_element(
                        by=By.CLASS_NAME,
                        value="product-price_productPrice__price--promo__Cxs_S",
                    ).text
                except NoSuchElementException:
                    prix = driver.find_element(
                        by=By.CLASS_NAME, value="product-price_productPrice__Uq0dh"
                    ).text
            ...
            if len(data["Designer"]) > n:
                break
        pageNumber += 1
        driver.get(
            f"https://fr.vestiairecollective.com/search/p-{pageNumber}/?q={query}"
        )
    driver.close()
